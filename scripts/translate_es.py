"""Translate EN HTML content to ES using Google Translate API."""
import sys, time, re
...
for i, ef in enumerate(files):
    ...
    try:
        ...
        if ok % 25 == 0:
            print(f'  {ok} done ({ok/elapsed*60:.1f}/min, {elapsed:.0f}s)')
...
elapsed = time.time() - t0
print(f'\nES: {ok} ok, {sk} skip, {err} err in {elapsed:.0f}s')
