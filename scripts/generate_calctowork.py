...
def normalize_trans(raw: dict) -> dict:
    ...
    return {
        ...
        "home_h1":         raw.get("home_h1", raw.get("site_tagline", "Free Online Calculators")),
        "home_title":      raw.get("home_title", f"{BRAND} \u2013 {raw.get('site_tagline', 'Free Online Calculators')}"),
        "home_browse_title": raw.get("home_browse_title", "Browse by Category"),
        "recent_title":    raw.get("recent_title", "Recently Used"),
        "favorites_title": raw.get("favorites_title", "My Calculators"),
        "search_placeholder": raw.get("search_placeholder", "Search calculators\u2026"),
        "search_no_results": raw.get("search_no_results", "No calculators found."),
...
date_published=DATE_PUBLISHED.get(calc.get("block", 0), "2025-01-01"),
    ...
    # ── 404 page ──────────────────────────────────────────────────────────────
    tmpl_404 = (TEMPLATES_DIR / "404.html.j2").read_text(encoding="utf-8") if (TEMPLATES_DIR / "404.html.j2").exists() else None
    if tmpl_404:
        write_file(PUBLIC / "404.html", tmpl_404)
    else:
        write_file(PUBLIC / "404.html", '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>404</title></head><body><h1>404</h1></body></html>')

    # ── Offline page ──────────────────────────────────────────────────────────
    tmpl_offline = (TEMPLATES_DIR / "offline.html.j2").read_text(encoding="utf-8") if (TEMPLATES_DIR / "offline.html.j2").exists() else None
    if tmpl_offline:
        write_file(PUBLIC / "offline.html", tmpl_offline)
