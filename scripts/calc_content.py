...
def build_article_from_facts(facts: dict, lang: str, calc_name: str = "") -> str:
    ...
    if not d:
        return ""
    H = {
        "es": ("{name}: la fórmula", "Ejemplo paso a paso", "¿Para qué sirve la {name}?", "Consejos prácticos"),
        "en": ("{name}: the formula", "Step-by-step example", "What is the {name} used for?", "Practical tips"),
        "fr": ("{name} : la formule", "Exemple pas à pas", "À quoi sert le {name} ?", "Conseils pratiques"),
        "pt": ("{name}: a fórmula", "Exemplo passo a passo", "Para que serve a {name}?", "Dicas práticas"),
        "de": ("{name}: Die Formel", "Schritt-für-Schritt-Beispiel", "Wofür wird der {name} verwendet?", "Praktische Tipps"),
        "it": ("{name}: la formula", "Esempio passo dopo passo", "A cosa serve il {name}?", "Consigli pratici"),
    }
    tips = {
